...
26行目でdict1を定義しています。これは **dict** 型という型の変数です。
...
26行目で作成したdict1を引数として使いましょう。

今回は左右方向のモータだけ動かせばいいので、リストの中の要素は1個だけです。

Q1-1で左右方向の関節名を取得したと思います。その名前を27行目のリストの要素として入れればOKです。

28行目には45°をラジアンに変換した値を入れましょう。

この関数は前問と同じくakari_controllerのライブラリに含まれており、23行目でakariという名前でこのライブラリを定義しています。
