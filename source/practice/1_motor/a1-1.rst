...
ヒントにもあったように、21行目でakariという名前でakari_controllerのライブラリを定義しています。
...
24行目でjoints_nameという変数に関節名を受け取ったので、これをprint関数で出力します。print(変数名)で出力できます。
...
29行目を入れておくと、ファイルがコマンドラインからスクリプトとして実行された場合にのみ処理を実行してくれます。

つまり他ファイルからimportしたときにこのコードの中身が自動実行されなくなります。

30行目にmain()という名前で実行する関数が指定されているので、15行目のmain()関数が実行されます。
