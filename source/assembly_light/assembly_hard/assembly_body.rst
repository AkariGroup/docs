...
手順
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
1. `(20) STS3215` に付属のサーボホーンを取り付ける。

.. image:: ../../images/assembly_light/body/body01-16.jpg
    :width: 400px

3. `(20) STS3215` を `(15) AKARI Light Body` 内部の溝に入れ、STS3215付属のタッピングビスで外側上部から4点ネジ留めする。

.. image:: ../../images/assembly_light/body/body01-18.jpg
    :width: 400px

1. 1 サーボモータケーブルの配線 で接続したケーブルを `(20) STS3215` に接続する。
...
手順
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
1. 先程取り付けた `(20) STS3215` のサーボホーンを、付属のM3ネジでネジ留めする。
...
手順
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
1. `(22) Raspberry Pi5用アクティブクーラー` の裏のシートを剥がす。

.. image:: ../../images/assembly_light/body/body01-20.jpg
    :width: 400px

2. `(22) Raspberry Pi5用アクティブクーラー` を `(18) Raspberry Pi5 8GB` に載せ、2箇所のピンを押して差し込み固定する。

.. image:: ../../images/assembly_light/body/body01-20.jpg
    :width: 400px

3. `(18) Raspberry Pi5 8GB`の FANコネクタのキャップを外し、 `(22) Raspberry Pi5用アクティブクーラー` のコネクタを差し込む。
